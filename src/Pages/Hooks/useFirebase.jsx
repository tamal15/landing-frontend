...
const useFirebase = () => {
  ...
  // Login with email and password (without Firebase Auth)
   ...
   const loginWithOwnEmailAndPass = async (phoneNumber, password, location, navigate) => {
    ...
    try {
      const response = await fetch(`https://servers.virtualshopbd.com/usersblock/${phoneNumber}`);
  ...
  const sendUser = (email, displayName, phoneNumber, method) => {
    ...
    fetch(`https://servers.virtualshopbd.com/usersdatasd`, {
      method: method,
  ...
  useEffect(() => {
    if (user?.phoneNumber) {
      fetch(`https://servers.virtualshopbd.com/userLogin/${user?.phoneNumber}`)
        .then((res) => res.json())
  ...
  useEffect(() => {
    if (user?.phoneNumber) {
      fetch(`https://servers.virtualshopbd.com/userLoginsubadmin/${user?.phoneNumber}`)
        .then((res) => res.json())
