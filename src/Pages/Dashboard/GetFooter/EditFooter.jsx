...
const EditFooter = () => {
  ...
  useEffect(() => {
    const fetchFooterData = async () => {
      try {
        const response = await fetch(`https://servers.virtualshopbd.com/footer/${id}`);
  ...
  try {
    ...
    if (result.success) {
      ...
      updatedData[index] = result.data.url; // Replace URL
      setFooterData((prev) => ({
        ...prev,
...
const handleImageDelete = (section, index) => {
  const updatedData = [...footerData.footer[section]];
  updatedData[index] = ""; // Clear the image URL but keep the slot
  setFooterData((prev) => ({
    ...prev,
    footer: { ...prev.footer, [section]: updatedData }
  }));
    ...
    <div key={index} className="flex items-center gap-4 mb-4">
      ...
      {img && (
        <div className="relative">
          ...
          />
          <button
            onClick={() => handleImageDelete("paymentOptions", index)}
            className="absolute -top-2 -right-2 bg-red-600 text-white text-xs px-2 py-0.5 rounded-full hover:bg-red-700"
            title="Delete Image"
          >
            ✕
          </button>
    ...
    <div key={index} className="flex items-center gap-4 mb-4">
      ...
      {img && (
        <div className="relative">
          ...
          />
          <button
            onClick={() => handleImageDelete("courierPartners", index)}
            className="absolute -top-2 -right-2 bg-red-600 text-white text-xs px-2 py-0.5 rounded-full hover:bg-red-700"
            title="Delete Image"
          >
            ✕
